...
import "./Navbar.css";
import {Tooltip, OverlayTrigger, NavDropdown, Nav} from "react-bootstrap";
...
const Navbar = props => {
    ...
    let projects = [
        "nav--rect",
        props.projects ? "nav--rect-active" : ""
    ]

    let skills = [
        "nav--rect",
        props.skills ? "nav--rect-active" : ""
    ]

    let contact = [
        "nav--rect",
        props.contact ? "nav--rect-active" : ""
    ]

    return (
        <>
        <Nb style={{top: "0%", padding: "0"}}>
            <NavDropdown title={`${lang} (${langShort})`} id="collasible-nav-dropdown dropdown-menu-align-right" className="language--dropdown" style={{position: "fixed", left: "1em", top: "1em", zIndex: "500",  borderRadius: "2em", border: "1px solid #ffffff", backgroundColor: "#232323"}}>
        ...
        <div className="nav--container" style={{zIndex: "500"}}>
            ...
            <OverlayTrigger overlay={<Tooltip>{props.t("header.contact", { framework: "react-i18next" })}</Tooltip>} placement="right">
